...
class AStar(object):
    def __init__(self, grid: OccupancyGrid, start_cell: tuple, goal_cell: tuple):
        self._grid = grid
    ...
    def solve(self) -> Optional[List[tuple]]:
        assert is_cell_empty(
            empty_cells=self._available_cells, cell=self._start_cell)
        assert is_cell_empty(
        ...
        while not self._open_set.empty():
            ...
            if(open_node.position == self._end_cell):
